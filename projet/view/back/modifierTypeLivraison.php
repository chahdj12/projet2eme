...
if (isset($_GET['id'])) {
    $typeLivraison = $typeLivraisonC->getTypeLivraisonById($_GET['id']);
}

if (isset($_POST["nom"], $_POST["description"], $_POST["cible"])) {
    $id = $_POST["id"];
    $nom = $_POST["nom"];
    $description = $_POST["description"];
    $cible = $_POST["cible"];

    $typeLivraison = new TypeLivraison($id, $nom,$description,$cible);
    $typeLivraison->setDescription($description);
    $typeLivraison->setCible($cible);

    $typeLivraisonC->modifierTypeLivraison($typeLivraison);
            ...
            <main>
                <div class="container-fluid px-4">
                    ...
                    <ol class="breadcrumb mb-4">
                        <li class="breadcrumb-item active">Modifier Type de Livraison</li>
                    </ol>

                    <div class="container mt-5">
                    <form method="post" action="" onsubmit="return validateForm()">
    <input type="hidden" name="id" value="<?= $typeLivraison['id'] ?>">

    <div class="form-group">
        <label for="nom">Nom</label>
        <input type="text" class="form-control" id="nom" name="nom" value="<?= $typeLivraison['nom'] ?>" >
    </div>
    <div class="form-group">
        <label for="description">Description</label>
        <input type="text" class="form-control" id="description" name="description" value="<?= $typeLivraison['description'] ?>" >
    </div>
    <div class="form-group">
        <label for="cible">Cible</label>
        <input type="text" class="form-control" id="cible" name="cible" value="<?= $typeLivraison['cible'] ?>" >
    </div>

    <button type="submit" class="btn btn-primary">Modifier</button>
    <a href="typeslivraisons.php" class="btn btn-danger">Retour à la liste</a>
</form>
                    </div>
                </div>
            </main>
